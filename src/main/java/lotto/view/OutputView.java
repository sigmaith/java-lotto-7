...
public class OutputView {
    ...
    private static final String EARNING_RATES = "총 수익률은 %.1f%%입니다.";
    ...
    public static void printWinningStatistics(Map<Integer, Integer> matchingRecords) {
        System.out.println(WINNING_STATISTICS);
        StringBuilder sb = new StringBuilder();
        for (int i = 3; i <= 7; i++) {
            sb.append(Rank.getRankByCode(i).getDescription()).append(matchingRecords.get(i)).append("개\n");
        }
        System.out.println(sb.toString());
    }

    public static void printEarningRates(Map<Integer, Integer> matchingRecords, Money money) {
        int earnings = 0;
        for (int i = 3; i <= 7; i++) {
